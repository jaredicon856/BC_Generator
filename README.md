...
| POST | `/api/webhook/fathom` | Automation entry point. Called by a Zapier/Make "Webhooks" action off Fathom's "New Transcript"/"New AI Summary" trigger. Body: `{ meetingTitle, clientName, clientKey, transcript, recordingId, presentedDate }` -- `clientKey` should be the client's email (stable across both calls); falls back to a normalized `clientName` if omitted, which is less reliable since display names can drift between calls. `meetingTitle` must contain "Strategy Call 1" or "Strategy Call 2" (also accepts "one"/"two"). Routes to the Authority Deck generator (Call 1) or the Battlecard generator with the stored Authority Deck as context (Call 2). Requires header `x-webhook-secret` matching `FATHOM_WEBHOOK_SECRET` env var. |
